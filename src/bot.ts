...
type MoveToPositionSchema = PositionSchema & {
  range: z.ZodOptional<z.ZodNumber>;
};

type MoveInDirectionSchema = {
  direction: z.ZodEnum<['forward', 'back', 'left', 'right']>;
  duration: z.ZodOptional<z.ZodNumber>;
};

type FindItemSchema = {
  nameOrType: z.ZodString;
};

type EquipItemSchema = {
  itemName: z.ZodString;
  destination: z.ZodOptional<z.ZodString>;
};

type PlaceBlockSchema = PositionSchema & {
  faceDirection: z.ZodOptional<z.ZodEnum<['up', 'down', 'north', 'south', 'east', 'west']>>;
};

type FindBlockSchema = {
  blockType: z.ZodString;
  maxDistance: z.ZodOptional<z.ZodNumber>;
};

type FindEntitySchema = {
...
function registerPositionTools(server: McpServer, bot: any) {
  ...
  server.tool(
    ...
    } satisfies MoveToPositionSchema,
    async (args: MoveToPositionParams): Promise<McpResponse> => {
      try {
  ...
  server.tool(
    ...
    "Make the bot look at a specific position",
    {
      ...
      y: z.number().describe("Y coordinate"),
      z: z.number().describe("Z coordinate"),
    } satisfies PositionSchema,
    async (args: PositionParams): Promise<McpResponse> => {
      try {
        const { x, y, z } = args;
  ...
  server.tool(
    ...
    } satisfies MoveInDirectionSchema,
    async (args: MoveInDirectionParams): Promise<McpResponse> => {
...
function registerInventoryTools(server: McpServer, bot: any) {
  ...
  server.tool(
    ...
    } satisfies FindItemSchema,
    async (args: FindItemParams): Promise<McpResponse> => {
      try {
  ...
  server.tool(
    ...
    } satisfies EquipItemSchema,
    async (args: EquipItemParams): Promise<McpResponse> => {
      try {
...
function registerBlockTools(server: McpServer, bot: any) {
  server.tool(
    ...
    } satisfies PlaceBlockSchema,
    async (args: PlaceBlockParams): Promise<McpResponse> => {
      try {
  ...
  server.tool(
    ...
    } satisfies FindBlockSchema,
    async (args: FindBlockParams): Promise<McpResponse> => {
      try {
...
function registerEntityTools(server: McpServer, bot: any) {
  server.tool(
    ...
    "Find the nearest entity of a specific type",
    {
      ...
      maxDistance: z.number().optional().describe("Maximum search distance (default: 16)")
    } satisfies FindEntitySchema,
    async (args: FindEntityParams): Promise<McpResponse> => {
      try {
...
function registerChatTools(server: McpServer, bot: any) {
  server.tool(
    ...
    } satisfies SendChatSchema,
    async (args: SendChatParams): Promise<McpResponse> => {
      try {
        const { message } = args;
        bot.chat(message);
